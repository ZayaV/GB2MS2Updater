...
namespace GB2MS2Updater
{
    ...
    public class GB2MS2Updater
    {
        ...
        private readonly UpdateFile[] ms2_update_data = new UpdateFile[]
        {
            ...
            new UpdateFile("ms2y",      "ms2yver",  "052-ms2.bin",  VersionType.Default,    1024,   0,     8,   0x00,     0)
        };
        ...
        private void ReadFirmwareFile(UpdateFile updateFile, bool setVersion)
        {
            if (updateFile.fileName != CurrentLoadedFileName)
            {
                ...
                if (byteCountToSend != FileLength)
                {
                    /* create a new padded array */
                    var newBytes = InitializeArray(byteCountToSend, updateFile.paddingByte);
                    Array.Copy(FileBytes, 0, newBytes, 0, FileLength);
        ...
        private void PrepareCS2FileVersionInfoToSend(UpdateFile updateFile)
        {
            ...
            if (byteCountToSend != FileLength)
            {
                ...
                Array.Copy(DataBytesToSendWithPadding, 0, newBytes, 0, FileLength);
                DataBytesToSendWithPadding = newBytes;
            }
        ...
        void SendFirmwareBlock(byte[] data, int length)
        {
            ...
            for (int i = 0, part = 0; i < length; i += 8, part++)
            {
                // Build Message Id containing current part number
                LastSentFrame = new CANMessage(BuildCANId(CANMaerklinCommand.Bootloader, 0x0300) + (byte)part, new byte[] { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 });
                ...
                switch ((CANMaerklinCommand)(e.CANMessage.Id & 0xFFFF0000UL))
                {
                    ...
                    case (CANMaerklinCommand.PingRsp):
                        ...
                        if ((e.CANMessage.Data.Length == 8) && (e.CANMessage.Data[0] == (byte)DeviceType))
                        {
                            ...
                            {
                                case DeviceType.GB2:
                                    Console.WriteLine("Found Track Box with ID 0x{0} Version {1}\n", CurrentDeviceId, deviceVersion);
                                    break;

                                case DeviceType.MS2:
                                    Console.WriteLine("Found Mobile Station 2 with ID 0x{0} Version {1}\n", CurrentDeviceId, deviceVersion);
                                    break;
                            }
                    ...
                    case CANMaerklinCommand.BootloaderRsp:
                        ...
                        if (PingReceived)
                        {
                            // DeviceType Type: 0x10 - GB2, 0x31 - MS2 (HW 2.0), 0x32 - MS2 (HW 2.1)
                            if (e.CANMessage.Data.Length == 8 && GetDeviceIdFromCanMessage(e.CANMessage) == CurrentDeviceId && ((e.CANMessage.Data[7] == 0x10) || (e.CANMessage.Data[7] == 0x31) || (e.CANMessage.Data[7] == 0x32)))
                            {
